...
def getenv(key):
    ...
    if val is None or not val:
        eprint(
            f"[!] Please set environment variable in dotenv file or environment: {key}"
        )
        sys.exit(1)
    return val
...
def user_limit(cursor) -> Iterator[List[User]]:
    while True:
        try:
            ...
            eprint(
                "[*] Iteration halted for user listing. Most likely the page limit has been hit"
            )
            break
...
def status_limit(cursor, username) -> Iterator[Status]:
    while True:
        try:
            ...
            eprint(
                f"[*] Iteration halted on status for {username}. Most likely the user is inactive."
            )
            break
...
def scan_cohort(api: API, page, date_cutoff: datetime) -> Cohort:
    ...
    for user in page:
        ...
        if not is_active_user:
            eprint(
                f"[**] Failed test {user_username}: {str(newest_created_at)} < {date_cutoff}"
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-e", "--env-file", type=str, help="A .env file containing API auth values"
    )
    parser.add_argument(
        "-d",
        "--days-past",
        type=int,
        default=30,
        help="List users who have not tweeted in past number of days. (Default: 30)",
    )
    parser.add_argument(
        "-P",
        "--max-pages",
        type=int,
        help="Maximum number of pages to search. Mostly for testing",
    )
    parser.add_argument(
